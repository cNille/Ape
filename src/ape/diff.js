...
import { isComponent, componentRender } from "./util";

// Diff
function changed(node1, node2) {
  const strCheck = typeof node1 === "string" && node1 !== node2;
  const numberCheck = typeof node1 === "number" && node1 !== node2;
  const boolCheck = typeof node1 === "boolean" && node1 !== node2;
  const nodeCheck = typeof node1 !== typeof node2;
  const typeCheck = node1.type !== node2.type;
  return typeCheck || strCheck || nodeCheck || numberCheck || boolCheck;
}
...
function diffChildren(newNode, oldNode, parentId) {
  ...
  for (let i = 0; i < patchesLength; i++) {
    ...
    if (
      newChild &&
      oldChild &&
      isComponent(newChild) &&
      isComponent(oldChild)
    ) {
      // If components
      newChild.props.id = parentId + "." + i;
      oldChild.props.id = parentId + "." + i;
      const newComp = new newChild.type(
        newChild.type,
        newChild.props,
        newChild.children
      );
      const oldComp = new oldChild.type(
        oldChild.type,
        oldChild.props,
        oldChild.children
      );

      const newRender = componentRender(newComp, newChild);
      const oldRender = componentRender(oldComp, oldChild);
      const childPatches = diff(newRender, oldRender, parentId) || [];
      patches.push(childPatches);
    } else {
      // Treat as html elements
      const d = diff(newNode.children[i], oldNode.children[i], parentId);
      if (d) {
        patches.push(d);
      }
    }
